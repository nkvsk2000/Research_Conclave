...
<body align="center" bgcolor="#ACEF84">
  ...
  <table align="center">
    ...
    <?php 
      ...
      foreach ($users as $result) {
        ...
        if (!empty($result["score1"]) && !empty($result["score2"]) ) {
           ...
           $avg=($result["score1"]+$result["score2"])/2;

           echo "<tr><td>".$result[0]."</td><td>".$result["title"]."</td><td>".$result["name"]."</td><td>".$rev1name."</td><td>".$rev2name."</td><td>".$result["score1"]."</td><td>".$result["score2"]."</td><td>".$avg."</td></tr>";
        }
      }
  ...
  <h2>Total No of graded Submission:<?php echo $completed ?></h2>
</body>
